...
const register = [
    ...
    body('lastName').exists().withMessage('lastName number is required').trim(),
    (req, res, next) => {
        validation(req, res, next);
    },
    body('country').exists().withMessage('country number is required').trim(),
    (req, res, next) => {
        validation(req, res, next);
    },
...
const getToken = [
    ...
    async (req, res, next) => {
        if (req.headers.authorization && req.headers.authorization.split(' ')[0] === 'Bearer') {
            ...
            try {
                const result = await authModel.getToken(data);
